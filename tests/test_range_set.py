...
class TestRangeSet(TestCase):
    ...
    for i in range(MIN_BOUND, MAX_BOUND + 1):
        QUERY_RANGES.extend([Range.at_most(i), Range.at_least(i), Range.less_than(i),
                             Range.greater_than(i), Range.closed(i, i),
                             Range.open_closed(i, i), Range.closed_open(i, i)])
        for j in range(i + 1, MAX_BOUND + 1):
            QUERY_RANGES.extend([Range.open(i, j), Range.open_closed(i, j),
                                 Range.closed_open(i, j), Range.closed(i, j)])
    ...
    def test_all_pair_ranges_enclosing(self):
        for query_range_1 in TestRangeSet.QUERY_RANGES:
            for query_range_2 in TestRangeSet.QUERY_RANGES:
                self._test_encloses(RangeSet.create_mutable()
                                    .add(query_range_1).add(query_range_2))
    ...
    def test_ignores_smaller_sharing_lower_bound(self):
        ...
        range_set.add(Range.closed(1, 4))
        self._test_invariants(range_set)
        self.assertEqual(ImmutableList.of([Range.closed(1, 6)]),
    ...
    def test_fill_hole_with_overlap(self):
        ...
        self._test_invariants(range_set)
        self.assertEqual(ImmutableList.of([Range.closed_open(1, 6)]),
                         ImmutableList.of(range_set.as_ranges()))
    ...
    def test_add_many_pairs(self):
        for a_low in range(0, 6):
            for a_high in range(0, 6):
                if a_low > a_high:
                    continue
                a_ranges = [Range.closed(a_low, a_high), Range.open_closed(a_low, a_high),
                ...
                for b_low in range(0, 6):
                    for b_high in range(0, 6):
                        if b_low > b_high:
                            continue
                        b_ranges = [Range.closed(b_low, b_high), Range.open_closed(b_low, b_high),
    ...
    def test_add_all(self):
        ...
        self.assertEqual(ImmutableList.of(range_set.as_ranges()),
    ...
    def _test_encloses(self, range_set: RangeSet[int]):
        ...
        for query_range in TestRangeSet.QUERY_RANGES:
            expected_to_enclose = any(x.encloses(query_range) for x in range_set.as_ranges())
    ...
    def _test_intersects(self, range_set: RangeSet[int]):
        for query in TestRangeSet.QUERY_RANGES:
            expect_intersects = any(r.is_connected(query) and not r.intersection(query).is_empty()
    ...
    def test_entry_above_below(self):
        ...
        value_at_or_below_reference = ((0, None), (1, 1), (2, 1), (3, 3), (4, 3), (5, 5), (6, 5),
                                       (7, 7), (8, 7), (9, 9), (10, 9), (200, 9))
        for (key, ref) in value_at_or_below_reference:
            self.assertEqual(_value_at_or_below(sorted_dict, key), ref)

        value_below_reference = ((0, None), (1, None), (2, 1), (3, 1), (4, 3), (5, 3), (6, 5),
                                 (7, 5), (8, 7), (9, 7), (10, 9), (200, 9))
        for (key, ref) in value_below_reference:
            self.assertEqual(_value_below(sorted_dict, key), ref)

        value_at_or_above_reference = ((0, 1), (1, 1), (2, 3), (3, 3), (4, 5), (5, 5), (6, 7),
                                       (7, 7), (8, 9), (9, 9), (10, None), (200, None))
